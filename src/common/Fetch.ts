...
export const Post = async <T>(path: string, data: { [x: string]: any }): Promise<T> => {
...
export const Get = async <T>(path: string, data?: { [x: string]: string }): Promise<T> => {
    const body = data ? Object.entries(data).map(([key, val]) => key + '=' + val).join('&') : ''
...
export const getUserAlarmProtocol = (protocol: string) => {
    return axios<Uart.ProtocolConstantThreshold>("getUserAlarmProtocol", { protocol })
}
...
export const getAlarmProtocol = (protocol: string) => {
    return axios<Uart.ProtocolConstantThreshold>("getAlarmProtocol", { protocol })
}
...
export const setUserSetupProtocol = (protocol: string, type: Uart.ConstantThresholdType, arg: any) => {
    return axios<Uart.ApolloMongoResult>("setUserSetupProtocol", { protocol, type, arg })
}
 ...
 * @returns 
 */
export const setAlias = (mac: string, pid: number, protocol: string, name: string, alias: string) => {
...
* @param Layout 
*/
export const setUserLayout = (id: string, type: string, bg: string, Layout: Uart.AggregationLayoutNode[]) => {
    return axios<Uart.ApolloMongoResult>("setUserLayout", { id, type, bg, Layout })
}


// V2 gps转高德gps
export const V2_API_Aamp_gps2autoanvi = (locations: string | string[], coordsys: 'gps' | 'mapbar' | 'baidu' = "gps") => {
    return axios<string | string[]>('util/AMap/GPS2autonavi', { coordsys, locations: Array.isArray(locations) ? locations.join("|") : locations })
}
 ...
 * @returns 
 */
const axios = async <T>(type: string, datas?: any): Promise<universalResult<{ data: T }>> => {
    try {
        const result = await Post<universalResult<{ data: T }>>("/api/" + type, datas)
...
async function AmapGet<T>(type: restype, params: Record<string, string>): Promise<T> {
    const result = await Get<universalResult<{ data: T }>>(ApiAddress + type, { key: AmapKey, ...params })
    return result.data
}