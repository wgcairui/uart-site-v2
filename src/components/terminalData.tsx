		...
		<>
			<Form layout="inline" style={{ marginBottom: 12 }}>
				<Form.Item label="查询时间段">
					<DatePicker.RangePicker value={[date[0], date[1]] as any} onChange={(_, d) => setDate(d.map((el) => moment(el)))}></DatePicker.RangePicker>
				</Form.Item>
			</Form>
			<Card style={{ maxHeight: 500, overflow: "auto" }} loading={loading}>
				<Timeline mode="left">
					{data.map(({ stat, timeStamp }, i) => (
						<Timeline.Item color={stat ? "red" : "green"} key={timeStamp + i + (stat ? "繁忙" : "空闲")} label={moment(timeStamp).format("MM-DD H:m:s:SSS")}>
							<p>设备状态:{stat ? "繁忙" : "空闲"}</p>
						</Timeline.Item>
					))}
				</Timeline>
					...
					{Object.entries(content).map(([key, val]) => (
						<Collapse.Panel header={val.title} key={key}>
							<Space wrap size="middle">
								{val.instructs.map((el) => (
									<Button shape="round" type="primary" key={el.value} onClick={() => setAt(el.value)}>
										{el.text}
									</Button>
								))}
							</Space>
			...
			<Card style={{ maxHeight: 500, overflow: "auto" }} loading={loading}>
				<Timeline mode="left">
					{data.map(({ msg, type, timeStamp }, i) => (
						<Timeline.Item color={type ? "blue" : "green"} key={timeStamp + i} label={moment(timeStamp).format("MM-DD H:m:s:SSS")}>
							<p>{msg || type}</p>
						</Timeline.Item>
					))}
				</Timeline>
...
export const TerminalRunData: React.FC<UserRunDataProps> = ({ mac, pid, user, closeFilter, OnUpdate }) => {
	...
	return !data ? (
		...
		<section>
			<Space>
				<span>{moment(data.time).isBefore(moment().startOf("day")) ? moment(data.time).format("YYYY/M/D H:mm:ss") : moment(data.time).format("H:mm:ss")}</span>
								...
								<span>
									{value + (!record.issimulate ? record.unit : "")}
									{
										<Tooltip color="cyan" title={`查看[${record.name}]的历史记录`}>
											<Link to={user ? `/main/devline/${mac + pid}?name=${record.name}` : `/root/node/terminal/devline?name=${record.name}&mac=${mac}&pid=${pid}`}>
	...
	return !Protocol.data ? (
		...
		<>
			...
			{data.length === 0 ? (
				...
				<Line slider={{ start: lineSlideStart }} autoFit loading={loading} xField="time" yField="value" seriesField="name" data={data} renderer="svg"></Line>
			)}
		</>
	);
};
