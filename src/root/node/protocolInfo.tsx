    ...
    return (
        <Form layout="inline" initialValues={re} onValuesChange={change} size="small" form={form}>
            ...
            <Form.Item name="bl" label="系数" extra="">
                <Select>
                    {
                        [1, 0.1, 10, 100, 1000, 0.01, 0.001].map(el => <Select.Option value={el} key={el}>{el}</Select.Option>)
                    }
                </Select>
            </Form.Item>
    ...
    return (
        <Card>
            <Space direction="vertical">
                {
                    data.map((el, i) => <ProtocolInstructFormResizeInput re={el} onChange={item => argModify(i, item)} key={el.name}></ProtocolInstructFormResizeInput>)
                }
                <Button type="primary" onClick={() => addforResize()}>添加参数</Button>
            </Space>
        </Card>
    )
}

interface props { Protocol: string }
/**
 * 协议指令修改页面
 * @param param0 
 * @returns 
    ...
    return (
        <Card>
            ...
            >
                ...
                <Form.Item name="resultType" label="结果集"
                    extra="hex:数据解析为Uint,utf8:数据解析为字符,bit:数据解析为2进制,"
                >
                    <Select>
                        {
                ...
                <Form.Item name="noStandard" label="非标协议" valuePropName="checked"
                    extra="非标准modbus协议"
                >
                    <Checkbox></Checkbox>
                </Form.Item>
                <Form.Item name="scriptStart" label="前处理脚本"
                ...
                <Form.Item name="scriptStart" label="后校验脚本"
                ...
                <Form.Item name="resize" label="解析"
...
export const ProtocolDes: React.FC<props> = ({ Protocol }) => {
    ...
    const remark = (val: string) => {
        modifyProtocolRemark(data.Protocol, val)
            .then(() => {
                fecth()
    ...
    const saveProtocol = () => {
        ...
        setProtocol(data.Type, data.ProtocolType, data.Protocol, instructs)
            .then(el => {
                loading()
                message.success('ok')
                fecth()
        ...
        loading ? <Spin />
            ...
            <>
                <Descriptions>
                    ...
                    <Descriptions.Item>
                        <Space>
                            <Button size="small" type="primary" onClick={() => addInstruct()}>添加指令</Button>
                            <Button size="small" type="primary" onClick={() => saveProtocol()} >保存协议</Button>
                        </Space>
                    </Descriptions.Item>
                </Descriptions>
                {
...
const ProtocolUpload: React.FC<props> = (props) => {
    ...
    return (
        <Space direction="vertical">
            ...
            {
                protocol && <Card>
                    <Button type="primary" onClick={() => updateP()}>更新协议</Button>
